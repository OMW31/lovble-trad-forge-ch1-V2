...
export function YieldCurveVisualizer() {
  ...
  const regime = spread < -0.5 ? "Inversion" : spread > 0.7 ? "Steepening" : "Normalisation";
  const W = 420;
  const H = 170;
  const min = 2;
  const max = 6;
  ...
  return (
    <WidgetFrame title="Yield Curve Visualizer" subtitle="Pilotez front-end et long-end pour comprendre inversion, steepening et risque cycle." badge="Rates curve">
      <div className="grid gap-5 lg:grid-cols-[1fr_240px]">
        <div>
          <div className="rounded-xl border bg-gradient-surface p-3">
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
              ...
              {tenors.map((t, i) => <text key={t} x={x(i)} y={H - 8} textAnchor="middle" className="font-mono" fontSize="10" fill="var(--muted-foreground)">{t}</text>)}
            </svg>
          </div>
          <div className="mt-4 space-y-4">
            <Slider label="Front-end 3M" value={front} set={setFront} />
            <Slider label="Long-end 10Y" value={long} set={setLong} />
          </div>
        </div>
        <div className="rounded-2xl border bg-surface p-5">
          ...
          <div className={cn("mt-3 rounded-lg border p-3 font-mono text-sm", spread < 0 ? "border-bear/40 bg-bear/10 text-bear" : "border-bull/40 bg-bull/10 text-bull")}>10Y-3M: {spread.toFixed(2)}%</div>
          <p className="mt-3 text-sm text-muted-foreground">Une courbe inversée signale une politique restrictive et un risque de ralentissement futur.</p>
        </div>
      </div>
    </WidgetFrame>
  );
